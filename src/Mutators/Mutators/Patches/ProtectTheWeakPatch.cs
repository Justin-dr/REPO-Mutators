...
namespace Mutators.Mutators.Patches
{
    internal class ProtectTheWeakPatch
    {
        ...
        private static IDictionary<string, object> _clients = new Dictionary<string, object>();

        static void OnMetadataChanged(IDictionary<string, object> metadata)
        {
            string? newBodyguardId = metadata.Get<string>(BodyGuardId);
            _clients = metadata.Get<IDictionary<string, object>>("clients") ?? new Dictionary<string, object>();
        ...
        static void RunManagerChangeLevelPostfix()
        {
            ...
            TemporaryItemUtils.DropAndRemoveMarkedItems(Mutators.ProtectTheWeakName);
        }
        ...
        private static void StartupLogic(string? excludedId = null)
        {
            ...
            Item? tranq = Items.AllItems.FirstOrDefault(x => "Tranq Gun".Equals(x.itemName));

            if (tranq)
            {
                GameObject? spawnedItem = Items.SpawnItem(tranq, bodyguard.transform.position, Quaternion.identity);
                ...
                if (spawnedItem != null)
                {
                    ...
                    if (viewID != null)
                    {
                        MutatorsNetworkManager.Instance.SendMetadata(
                            MutatorSettings.ProtectTheWeak.NamespacedName,
                            BuildMeta(bodyguard, viewID.Value)
                        );
        ...
        private static void AssignBodyguard(IDictionary<string, object> metadata)
        {
            if (!string.IsNullOrEmpty(_bodyGuardId))
            {
                ...
                MutatorAnnouncingBag announcingBag = MutatorAnnouncingBag.Instance;
                ...
                bool announcementPresent = announcingBag.TryGetAnnouncement(
                    MutatorSettings.ProtectTheWeak.NamespacedName, out MutatorAnnouncement? announcement
                );
                ...
                if (initDone)
                {
                    if (_bodyGuardId == playerAvatar.steamID)
                    {
                        ...
                        {
                            string? description = metadata.Get<string>("descriptionBodyguard");
                    ...
                    else
                    {
                        if (announcementPresent)
                        {
                            string? description = metadata.Get<string>("description");
                            announcement!.AddOrUpdateSegment(new MutatorAnnouncementDescriptionSegment(
                                "role",
                                10,
                                 description == null ? string.Empty : $"\n{description}")
                ...
                if (announcementPresent && announcingBag.MoveTo(announcement!))
                {
        ...
        private static IDictionary<string, object> BuildMeta(PlayerAvatar bodyguard, int tranqGunId = 0)
        {
            ...
            return meta.WithMutator(MutatorSettings.ProtectTheWeak.NamespacedName);
        }
